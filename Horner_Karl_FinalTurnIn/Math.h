...
VERTEX VertexMultMatrix(VERTEX& v, MATRIX& m)
{
	VERTEX temp = v;
...
unsigned int TwoToOne(unsigned int X, unsigned int Y, unsigned int W)
{
...
//	return c;
//}
void RotateX(VERTEX& c, float rad)
{
	rad = rad * (M_PI / 180);
...
VERTEX RotateY(VERTEX& c, float rad)
{
	rad = rad * (M_PI / 180);
	float multMe[4] = {c.pos.x, c.pos.y, c.pos.z, c.pos.w};
...
MATRIX Mult4x4(MATRIX& a, MATRIX& b)
{
	MATRIX temp = a;
...
MATRIX Mult4x4notRef(MATRIX a, MATRIX b)
{
	MATRIX temp = a;
	temp.x[0] = a.x[0] * b.x[0] + a.x[1] * b.y[0] + a.x[2] * b.z[0] + a.x[3] * b.w[0];
	temp.y[0] = a.y[0] * b.x[0] + a.y[1] * b.y[0] + a.y[2] * b.z[0] + a.y[3] * b.w[0];
...
unsigned int LerpTheColors(unsigned int a, unsigned int b, unsigned int c, VERTEX barry)
{
	unsigned int b1 = (a & 0x000000FF);
	auto g1 = (a & 0x0000FF00) >> 8;
	auto r1 = (a & 0x00FF0000) >> 16;
...
VERTEX CartesianConversion(VERTEX& convert)
{
