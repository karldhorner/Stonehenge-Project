...
inline void PlotAPixel(int x, int y, float z, unsigned int color)
{
	if ((x >= 0 && y >= 0))
	{
...
inline void reverseValues(unsigned int& reverseIt)
{
...
inline void PS_White(unsigned int& makeWhite)
{
	makeWhite = 0xFFFFFFFF;
}

inline void PS_Green(unsigned int& makeGreen)
{
	makeGreen = 0xFF00FF00;
}

inline void PS_Red(unsigned int& makeRed)
{
	makeRed = 0xFFFF0000;
}

inline void PS_Yellow(unsigned int& makeYellow)
{
	makeYellow = 0xFFFFFF00;
}

inline void PS_Blue(unsigned int& makeBlue)
{
	makeBlue = 0xFF0000FF;
}
...
inline void DrawLine(VERTEX& a, VERTEX& b)
{
	auto copyA = a;
	auto copyB = b;
	if(VertexShader)
...
inline float ImplicitLineEq(VERTEX a, VERTEX b, VERTEX p)
{
	return (((a.pos.y - b.pos.y) * p.pos.x) + ((b.pos.x - a.pos.x) * p.pos.y) + (a.pos.x * b.pos.y) - (a.pos.y * b.pos.x));
}
//Make barycentric formula
inline VERTEX findBarry(VERTEX a, VERTEX b, VERTEX c, VERTEX p)
{
...
inline void fillTriangle(VERTEX& a2c, VERTEX& b2c, VERTEX& c2c, VERTEX color)
{
	VERTEX a = a2c;
	VERTEX b = b2c;
	VERTEX c = c2c;
